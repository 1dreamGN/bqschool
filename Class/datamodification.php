...
if ($do = $_POST['do']) {
    ...
    $gxqm=safestr($_POST['gxqm']);
		...
		if ($stmt->fetch(PDO::FETCH_ASSOC)) {
			echo "<script language='javascript'>alert('手机号已存在！');window.location.href='/Class/datamodification.php';</script>";
		} else {
			$db->query("update {$prefix}users set {$set} where uid='{$userrow['uid']}'");
			echo "<script language='javascript'>alert('修改成功');window.location.href='/Class/datamodification.php';</script>";
		}
	}
}
C('webtitle', $userrow['user'] . '-资料修改');
C('pageid', 'datamodification');
