...
if($_SERVER['HTTP_REFERER']!="http://{$domain}/Class/qqmusic.php")exit("Error".$_SERVER['HTTP_REFERER']);
if ($_GET['type'] == "music") {
    $qq = $_POST['qq'];
    if (!$qq) {
        exit('{"code":-1,"msg":"QQ不能为空"}');
    } else {
        $data = get_curl('http://api.qq-admin.cn/?mod=QQMusicJs&qq='. $qq);
		exit('{"code":1,"msg":"加速成功！"}');
    }
}
?>
