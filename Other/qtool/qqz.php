...
if($_SERVER['HTTP_REFERER']!="http://{$domain}/Class/quanz.php")exit("Error".$_SERVER['HTTP_REFERER']);
if ($_GET['type'] == "quanz") {
    $qq = $_POST['qq'];
    if (!$qq) {
        exit('{"code":-1,"msg":"QQ不能为空"}');
    } else {
        $data = get_curl('http://lqq.25ak.com/quan.php?jx='. $qq);
		exit('{"code":1,"msg":"刷圈圈赞99+成功!"}');
    }
}
?>
